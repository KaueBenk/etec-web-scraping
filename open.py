...
url = 'https://www.vestibulinhoetec.com.br/classificacao-final/lista.asp?ensino=4&codetec=015.00S&codescolacurso=1637&n=&o=1'
...
if get_html_from_url:
    ...
    while True:
        ...
        if path.exists(dir_school) == True:
            chdir(dir_school)
            arquivo = open(f'{course}.csv', 'x')
        elif getcwd() == dir_school:
            arquivo = open(f'{course}.csv', 'x')
        elif not path.exists(course):
            mkdir(school)
            chdir(dir_school)
            arquivo = open(f'{course}.csv', 'x')
